...
class RobotGymEnv(Supervisor, gym.Env):
    def __init__(self, max_episode_steps=10_000):
        ...
        self.goal_candidates = [
            ...
            np.array([1.24, -0.45], dtype=np.float32),
            np.array([2.0, -0.5], dtype=np.float32),
            np.array([2.3, -1.15], dtype=np.float32),
            np.array([2.27, 1.57], dtype=np.float32),
            np.array([2.3, 2.3], dtype=np.float32),
            np.array([3.15, 2.4], dtype=np.float32),
            np.array([0.7, 1.0], dtype=np.float32),
            np.array([1.43, 0.9], dtype=np.float32),  
    ...
    def reset(self, *, seed=4, options=None):
        ...
        success_rate = (self.success / self.episode_number) * 100.0
        print(f"Episode {self.episode_number} Success rate: {success_rate:.2f}%") #print sucess rate
...
def main():
    ...
    model_path = "models/TD3/best_model_630000.zip"  #load model
    ...
    for _ in range(10_000): #run inferece
